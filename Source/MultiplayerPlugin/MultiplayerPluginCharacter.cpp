...
void AMultiplayerPluginCharacter::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
		...
		if (GEngine)
		{
			...
			if(World)
			{
				...
				GEngine->AddOnScreenDebugMessage(
					...
					15.f,
					FColor::Blue,
					FString::Printf(TEXT("Test"), *SessionName.ToString()));
				World->ServerTravel(FString("/Game/ThirdPerson/Maps/Lobby2?listen"));
			}
		}
	}
	else
	{
		if(GEngine)
		{
			GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Red, FString(TEXT("Failed Create Session")));
		}
	}
